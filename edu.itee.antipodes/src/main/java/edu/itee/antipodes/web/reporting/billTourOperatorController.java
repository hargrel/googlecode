...
public class billTourOperatorController {
	...
	public ModelAndView post(@ModelAttribute("billingTourOperator") BillingTourOperators bto,
			BindingResult result) {
		
		validator.validate(bto, result);
		if (result.hasErrors()) { return new ModelAndView("billTourOperator"); }
		...
		try {
			...
			model.put("ReportTitle", "Billing report for " + to.getOperatorName());
